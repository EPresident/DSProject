...
Naturalmente si vuole evitare che le richieste di prenotazione interferiscono tra di loro (e soprattutto evitare che lo stesso posto venga prenotato più volte). Nel caso in cui venga richiesto un posto che risulta occupato, la compagnia aerea dovrebbe proporre un posto alternativo (se possibile).
...
\paragraph{Considerazioni} Si suppone che il sistema venga operato da umani (gli impiegati delle agenzie di viaggio), perciò è lecito ritenere che le probabilità di interferenza tra prenotazioni sia basso.
...
\begin{itemize}
	...
	\item Una volta confermata una prenotazione per un posto aereo, questa non deve essere più annullata.
\end{itemize}
...
\begin{itemize}
	...
	\item Tempi di risposta contenuti: se una transazione non viene completata entro 40 secondi, viene abortita dal client.
\end{itemize}
